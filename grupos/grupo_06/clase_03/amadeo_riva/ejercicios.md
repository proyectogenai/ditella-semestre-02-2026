...
Le pedí en un chat limpio que me diera un prompt para una imagen de detalle
de producto de Halcón, sin mencionar la skill ni el nombre "pedro".
...
Cuando la probé en un chat limpio, la skill no cargaba: yo ya la había
instalado, pero el agente seguía sin verla. Le consulté qué estaba pasando y
me dijo que tenía que cerrar la sesión y volver a abrirla para que el agente
recién levantara la skill recién creada.

**Volví a probar y ahora:**

Le hice caso: cerré la sesión y volví a abrirla, y ahí sí la skill cargó
sola al pedirle algo de su tema. Mi síntoma en C.4 fue "no la carga nunca",
pero era un tema de reiniciar la sesión — no de que el `description`
estuviera mal escrito. En el próximo intento voy a pedirle a otra persona
que pruebe con una frase que no mencione la skill directamente, para
confirmar que el `description` la dispara bien de entrada.
